...
import { RolesGuard } from 'src/common/guards/guards/auth/role.guard';
import { JwtGuard } from 'src/common/guards/guards/auth/auth-guards.guard';

@Controller('v1/user-role')
@UseGuards(JwtGuard, RolesGuard)
export class UserRoleController {
  constructor(private readonly existsUserIdAndRoleId: ExistsByRoleIdAndUserIdUseCase) {}

  @Get('/exists/:userId/:roleId')
  async exists(@Param('userId') userId: string, @Param('roleId') roleId: string) {
