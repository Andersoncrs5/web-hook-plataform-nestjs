...
export class UserRepository implements IUserRepository {
    ...
    async findAll(filter: UserFilter, pageable: Pageable<UserSort>): Promise<Page<User>> {
        ...
        if (filter.id) {
            conditions.push(eq(users.id, filter.id));
        }
        ...
        if (filter.fullName) {
            conditions.push(ilike(users.fullName, `%${filter.fullName}%`));
        }
        ...
        if (filter.createdAtMin) {
            conditions.push(gte(users.createdAt, filter.createdAtMin));
        }

        if (filter.createdAtMax) {
            conditions.push(lte(users.createdAt, filter.createdAtMax));
        }

        if (filter.updatedAtMin) {
            conditions.push(gte(users.updatedAt, filter.updatedAtMin));
        }

        if (filter.updatedAtMax) {
            conditions.push(lte(users.updatedAt, filter.updatedAtMax));
        }
        ...
        const sortColumn =
            sortableColumns[pageable.sortBy] ?? users.createdAt;
        ...
        const result = await this.database.connection
            ...
            .limit(pageable.size)
            .offset((pageable.page - 1) * pageable.size) as User[];
