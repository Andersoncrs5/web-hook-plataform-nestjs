...
export class BaseIntegrationTest {
  ...
  static async setupAll(): Promise<void> {
    ...
    this.postgres = await new PostgreSqlContainer('postgres:18-alpine')
      .withDatabase('web_hook_db')
      .withUsername('pochita')
      .withPassword('pochita')
      .start();

    this.redis = await new RedisContainer('redis:8-alpine').start();
    ...
    process.env.JWT_SECRET = 'integration-test-secret-key-change-me-very-long-256-bit';
    ...
    process.env.IGNORE_EXPIRATION = 'false';
    ...
    const migrationsFolder = path.resolve(__dirname, '../../src/infra/database/migrations');

    const migrationClient = postgres(process.env.DATABASE_URL!, { max: 20 });
